...
def search_card(search_name):
    for monster_name, monster_info in monsters.items():
        if search_name.lower() in monster_name.lower():
            return monster_info
        for item_name, item_price in monster_info.items():
...
def search():
    search_term = easygui.enterbox("Enter the name of the monster you want "
                                   "to search for:",
                                   title='End Search')
    ...
    if result:
        ...
        easygui.msgbox("Monster not found.", "Search Result")
